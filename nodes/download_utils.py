...
class DownloadManager:
    ...
    def download_with_progress(
        url, save_path, progress_callback=None, params=None, chunk_size=1024 * 1024
    ):
        response = requests.get(url, stream=True, params=params)
        response.raise_for_status()

        total_size = int(response.headers.get("content-length", 0))
        filename = DownloadManager._get_filename(response, url)  # Sanitized filename
    ...
    def _get_filename(response, url):
        ...
        parsed_url = urlparse(url)  # noqa: F821
